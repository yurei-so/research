...
for (const family of manifest.families) {
  ...
  fs.writeFileSync(path.join(graphDirectory, "index.html"), graphPage
    ...
    .replace('Two strictly separate views of the published record: authored provenance and corpus-relative research attention.', 'A corpus-relative attention map with authored lineage shown only for the selected note.')
    .replace('<div class="map-modes" aria-label="Detailed visualization"><button type="button" data-map-view="attention" aria-pressed="true">ATTENTION HEATMAP</button><button type="button" data-map-view="provenance" aria-pressed="false">PROVENANCE WEB</button><p id="view-disclosure">Human-authored relationships only. Proximity is not used to create edges.</p></div>', '<div class="map-modes"><strong>ATTENTION HEATMAP</strong><p id="view-disclosure">Semantic proximity is approximate. Only selected-note lineage is human-authored.</p></div>')
    .replace(/<svg class="project-map"[\s\S]*?<\/svg>(?=<svg class="attention-map")/, "")
    .replace('<button id="trace-lineage" type="button" aria-pressed="false">TRACE LINEAGE</button>', '')
    ...
    .replace('<div class="graph-toolbar" aria-label="Map controls">', '<div class="graph-toolbar" aria-label="Map controls"><div class="map-layout-switch" aria-label="Map layout"><button type="button" data-page-view="standard" aria-pressed="false">SCROLL</button><button type="button" data-page-view="beta-fit" aria-pressed="true">FIT SCREEN</button></div>')
    .replace('<span>DRAG SCROLLBARS OR PAN WITH TOUCH</span>', '<span class="map-pan-hint">DRAG MAP TO EXPLORE</span>')
    .replace('<div class="graph-stage"', '<nav class="mobile-map-instrument" aria-label="Mobile map navigation"><span class="mobile-record-readout">PUBLISHED RECORD · <b id="mobile-record-position">—</b></span><button id="mobile-catalog-next" type="button" aria-label="Next labnote in catalog">+</button><button id="mobile-lineage-previous" type="button" aria-label="Earlier provenance relation">←</button><button id="mobile-lineage-next" type="button" aria-label="Later provenance relation">→</button><button id="mobile-catalog-previous" type="button" aria-label="Previous labnote in catalog">−</button></nav><div class="graph-stage"')
    .replace('<svg class="attention-map"', '<div class="map-world"><svg class="attention-map"')
    .replace('</svg></div></div><aside class="graph-inspector"', '</svg></div></div></div><aside class="graph-inspector"')
    .replace('<aside class="graph-inspector" aria-live="polite">', '<aside class="graph-inspector" aria-live="polite"><button id="mobile-sheet-toggle" type="button" aria-expanded="false"><span aria-hidden="true"></span><b>EXPAND DETAIL</b></button>')
    .replace("</head>", `<link rel="alternate" type="application/json" href="index.json" title="Compact agent overview"></head>`)
    .replace('<script type="module" src="../../assets/project-graph.js">', `<template id="project-graph-data">${inlineJson(graphData)}</template><script type="module" src="../../assets/project-graph.js">`)
