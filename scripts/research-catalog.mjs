...
<footer><span>YUREI RESEARCH · <a href="${repositoryUrl}">SOURCE</a> · <a href="https://github.com/yurei-so">GITHUB</a></span><span>REV ${manifest.source_revision}</span></footer><script type="module" src="../../assets/labnote.js"></script></body></html>`;
...
for (const family of manifest.families.filter((entry) => entry.has_graph)) {
  ...
  const familyNoteList = [...notes].sort((a, b) => b.date.localeCompare(a.date) || b.id.localeCompare(a.id)).map((note) =>
    `<li class="family-note" data-note="${escapeHtml(note.id)}" data-search="${escapeHtml(`${note.id} ${note.title} ${note.question} ${note.tags.join(" ")}`.toLowerCase())}" data-outcome="${escapeHtml(note.outcome)}"><a href="../../${escapeHtml(note.href)}"><span class="family-note-id">${escapeHtml(note.id)}</span><strong>${escapeHtml(note.title)}</strong><span class="family-note-meta"><time datetime="${escapeHtml(note.date)}">${escapeHtml(note.date)}</time> · <span data-outcome="${escapeHtml(note.outcome)}">${escapeHtml(note.outcome)}</span></span><span class="family-note-question">${escapeHtml(note.question)}</span></a></li>`).join("");
