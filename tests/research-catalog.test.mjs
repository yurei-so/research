...
test("family view defaults to the attention map with a synchronized note rail", () => {
  ...
  assert.match(client, /const duration = 260/);
  ...
  assert.match(styles, /\.map-world \{[^}]+padding: 50vh 50vw/s);
  assert.match(styles, /data-page-view="beta-fit"\] \.map-world \{ padding: 0; \}/);
  assert.match(styles, /outline: 1px solid #343b4c/);
  assert.match(client, /panCameraTo\(\s*offset\.left \+ x/s);
